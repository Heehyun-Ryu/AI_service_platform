...
function App() {
    ...
    // isStreaming 상태를 동기화하는 ref 추가
    const isStreamingRef = useRef(isStreaming);

    // isStreaming 상태가 바뀔 때 ref도 같이 갱신하도록 useEffect 추가
    useEffect(() => {
    ...
    const startCamera = async () => {
        try {
            ...
            isStreamingRef.current = false; // 에러시에도 동기화
        }
    };
    ...
    const stopCamera = () => {
        ...
        setIsStreaming(false);
        isStreamingRef.current = false; // ref도 false로 변경
    ...
    const detectObjects = async () => {
        if (!videoRef.current || !isStreamingRef.current || isProcessing) {
            return;
        }
